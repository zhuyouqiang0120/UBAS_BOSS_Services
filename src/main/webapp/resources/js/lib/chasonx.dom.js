			...
			if(opt.item && opt.item.length > 0){
				...
				for(var i = 0;i < opt.item.length;i ++){
					...
					if(_item.type == 'input'){
						...
						line += '<domItem><input id="'+ _item.name +'" name="'+ _item.name +'" '+ (_item.attr || '') +' value="'+ (_item.value || '') +'" type="text" onclick="WdatePicker({dateFmt:\'yyyy-MM-dd\',realFullFmt:\'%Date\'})" class="inputText Wdate"/></domItem>';
