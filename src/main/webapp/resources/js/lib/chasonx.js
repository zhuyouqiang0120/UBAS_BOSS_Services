...
Chasonx.Alert = function(options){
	...
	if(typeof(options.alertType) == 'string'){
		...
		if(options.alertType == 'normal') leftHtml = '<div style="'+ leftstyle + 'color:#26A2E8;' + '">？</div>';
		else if(options.alertType == 'warning') leftHtml = '<div style="'+ leftstyle + 'color:#EF7107;' + '">！</div>';
		else if(options.alertType == 'error') leftHtml = '<div style="'+ leftstyle + 'color:#FF3333;' + '">×</div>';
...
Chasonx.prototype = {
	...
	/**
	* Dialog Box
	* options {title:'标题',html:'<div></div>',width:200,height:100,modal:true,success:function(){},close:true,okText:'保存',cancelText:'关闭',alertType:'warning'}
	*/
		...
		if(options.isdialog){
			var	 mf = 'display:block;height:60px;background-color:#f7f6f6;text-align:right;vertical-align:middle;line-height:60px;padding-right:10px;position:relative;',
				...
				if(typeof options.success == 'function'){
					var	ok   = 'position:absolute;min-width:60px;width:auto !important;height:30px;right:80px;top:15px;vertical-align:middle;line-height:30px;background-color:'+  this.attr.alertType[options.alertType] +';color:#FFF;border:none;cursor:pointer;' + this.attr.baseCss,
...
/**
*get Window Size; create Element; get Browser Type;
*/
var ChasonTools = {
	 /**
	  * 取窗口大小
	 ...
	 getObjectSize : function(o){
	 	if(!o) return [];
	 	return [o.offsetWidth,o.offsetHeight,this.getOffsetLeft(o),this.getOffsetTop(o)];
	 },
	 /**
	  * 取dom距左距离
	  * @param e
