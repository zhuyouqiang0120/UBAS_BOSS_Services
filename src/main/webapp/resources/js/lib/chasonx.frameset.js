...
*  UpdateTime : 2016-09-19 属性添加
*/
;(function(window){
	/**
	*类似于frameset的页面框架
	* window ： {
					top: { id : '设置的ID' ,title : '显示标题', height : '显示高度', html : '显示内容',bgcolor : '',borderColor : '',titleBgColor : '',color : ''},
	  ...
	  targetPanel : '模板的载体，默认为body'，
	  mainPanel   ： '四个块的主块，不传会默认创建一个'
	*
	*/
	...
	ChasonxFrameset.prototype = {
		...
		style  :  {
			...
			top     : 'width:100%;position:absolute;border-width:1px;border-style: solid;transition: all 0.3s ease;  -webkit-transition: all 0.3s ease;  -moz-transition: all 0.3s ease;',
			bottom  : 'width:100%;border-width:1px;border-style: solid;position:absolute;transition: all 0.3s ease;  -webkit-transition: all 0.3s ease;  -moz-transition: all 0.3s ease;',
			left 	: 'border-width:1px;border-style: solid;position:absolute;left:0px;display:inline-block;transition: all 0.3s ease;  -webkit-transition: all 0.3s ease;  -moz-transition: all 0.3s ease;',
			right   : 'border-width:1px;border-style: solid;position:absolute;right:0px;display:inline-block;transition: all 0.3s ease;  -webkit-transition: all 0.3s ease;  -moz-transition: all 0.3s ease;',
		...
		init : function(){
			...
			if(this.window.top){
				...
				_csss += "background-color:" + (this.window.top.bgColor || this.style.defBgColor) + ";" +
				  		 "border-color:" + (this.window.top.borderColor || this.style.defBorderColor) + ";";
				if(top){
			...
			if(this.window.bottom){
				...
				_csss += "background-color:" +  (this.window.bottom.bgColor || this.style.defBgColor)  + ";" +
						 "border-color:" + (this.window.bottom.borderColor || this.style.defBorderColor) + ";";
				if(bottom){
			...
			if(this.window.left){
				...
				_csss += "background-color:" +  (this.window.left.bgColor || this.style.defBgColor)  + ";" +
						 "border-color:" + (this.window.left.borderColor || this.style.defBorderColor) + ";"; 
			...
			if(this.window.right){
				...
				_csss += "background-color:" +  (this.window.right.bgColor || this.style.defBgColor)  + ";" +
						 "border-color:" + (this.window.right.borderColor || this.style.defBorderColor) + ";";
				if(right){
...
*/
	function ChasonxDragBox(option){
		...
		this.height = option.height || '100px';
	...
	ChasonxDragBox.prototype = {
		...
		style     	: {
			  ...
			  line  : 'cursor:e-resize;display:inline-block;height:100%;position:absolute;right:0px;width:3px;top:0px;bottom:0px;'
		},
		init : function(){
			...
			if(!main) return ;
			main.setAttribute('style',this.style.main);
		...
		bindHandler : function(){
			for(var i = 0,len = this.eleContainer.length;i < len; i++){
					this.bind(this.eleContainer[i].Line, (i < (len - 1)?this.eleContainer[i + 1].Td:null),this);
			}
		},
		bind : function(O,nextTd,_this){
			if(O == null ) return;
			if(window.addEventListener){
			    O.addEventListener('mousedown',function(){ _this._handler(this,nextTd);},false);
			}else {
			    O.attachEvent('onmousedown',function(){ _this._handler(this,nextTd); });
			}
		},
