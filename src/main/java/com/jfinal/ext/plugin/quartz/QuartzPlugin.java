...
public class QuartzPlugin implements IPlugin {
	...
	private void startJobs() {
		...
		for (Map.Entry<Job, String> entry : set) {
			...
			JobDetail jobDetail;
			CronTrigger trigger;
			// JobDetail and CornTrigger are classes in 1.x version,but are
			// interfaces in 2.X version.
			if (VERSION_1.equals(version)) {
				...
				jobDetail = Reflect.on("org.quartz.JobBuilder").call("newJob", job.getClass())
						.call("withIdentity", jobClassName, jobClassName).call("build").get();
